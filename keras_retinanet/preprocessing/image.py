...
def adjust_transform_for_image(transform, image, relative_translation):
    ...
    The linear part of the transformation will adjusted so that the origin of the transformation will be
...
def apply_transform(matrix, image, params):
    ...
    The matrix is interpreted such that a point (x, y) on the original image is moved to
    transform * (x, y) in the generated image.
    Mathematically speaking, that means that the matrix is a transformation from the transformed image space to
...
def resize_image(img, min_side=800, max_side=1333):
    ...
    Args
        ...
        max_side: If after resizing the image's max side is above max_side, resize until the max side is
        equal to max_side.
