...
def get_labels(metadata_dir, version='challenge2018'):
    if version == 'challenge2018':
...
def generate_images_annotations_json(main_dir, metadata_dir, subset, cls_index, version='challenge2018'):
...
class OpenImagesGenerator(Generator):
    def __init__(
            self, main_dir, subset, version='challenge2018',
    ...
    def __filter_data(self, id_to_labels, cls_index, labels_filter=None, parent_label=None):
        ...
        if parent_label is None:
            ...
            parent_cls = None
            for i, lb in iter(id_to_labels.items()):
                if lb == parent_label:
                    parent_id = i
                    for c, index in iter(cls_index.items()):
                        if index == parent_id:
                            parent_cls = c
                    break
        ...
        id_map = dict([(ind, i) for i, ind in enumerate(iter(children_id_to_labels.keys()))])
        ...
        children_id_to_labels = dict([(id_map[i], l) for (i, l) in iter(children_id_to_labels.items())])
